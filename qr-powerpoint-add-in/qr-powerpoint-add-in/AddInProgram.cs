...
namespace qr_powerpoint_add_in
{
    public partial class AddInProgram
    {
        ...
        {
            this.Application.SlideSelectionChanged += RefreshMemory;

            _keyboardListener = new KeyboardHookListener(new AppHooker());
            _keyboardListener.Enabled = true;
            _keyboardListener.KeyDown += new KeyEventHandler(CreateImageClipboard);
        }
        ...
        private void CreateImageClipboard(object sender, KeyEventArgs e)
        {
            if (e.Control && e.KeyCode == Keys.V)
            {
                if (Clipboard.ContainsText())
                {
                    ...
                    if (_encodedUrl != text && Regex.IsMatch(text, FORMS_REGEX))
                    {
        ...
        private void RefreshMemory(PowerPoint.SlideRange slideRange)
        {
            if (_encodedUrl != null)
            {
                _encodedUrl = null;
